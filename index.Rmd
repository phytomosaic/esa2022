...
```{r the-matrix, eval=TRUE, echo=FALSE}
load('./data/veg.rda')      # load the object
veg$spe[1:10, c(8,9,18,54)] # head of species matrix
```
...
par(mfrow=c(1,1), las=1, oma=c(0,0,0,0), mar=c(2.7,2.1,0.1,0.1),
...
.pull-right.w45[
```{r diss-02-invisible-eval, eval = TRUE, echo = FALSE, fig.height = 5.0, fig.width = 5.0}
# this code should not render
snk <- capture.output(D <- stepacross(d, 'shortest', toolong = 1))
plot(d, D, xlab = 'Original', ylab = 'Stepacross', las=1, oma=c(0,0,0,0), mar=c(3.1,3.1,0.1,0.1), bty='L', mgp=c(2,0.3,0), tcl=0.2) 
...
.pull-right.w40[
```{r double-zero-05, eval = TRUE, echo = FALSE, fig.height = 4.5, fig.width = 4.5}
ecole::set_par(1, mar=c(1.5,1.5,0,0), pty='m')
...
u <- ecole::colvec(1:NROW(spe))
ecole::set_par(6)
plot(scores(pca, display='si'), col=u, xlab='PCA1',   ylab='PCA2')
plot(scores(ca,  display='si'), col=u, xlab='CA1',    ylab='CA2')
plot(scores(pco, display='si'), col=u, xlab='PCO1',   ylab='PCO2')
plot(scores(rda, display='si'), col=u, xlab='RDA1',   ylab='RDA2')
plot(scores(cca, display='si'), col=u, xlab='CCA1',   ylab='CCA2')
plot(scores(dbr, display='si'), col=u, xlab='dbRDA1', ylab='dbRDA2')
```
...
par(mfrow=c(1,2), las=1, oma=c(0,0,0,0), mar=c(4.1,4.1,1.1,0), bty='L', mgp=c(2,0.3,0), tcl=0.2)
plot(nms)
stressplot(nms)
```
...
```{r permanova-00, eval = TRUE, echo = TRUE, fig.height = 4.5, fig.width = 5.0, fig.align='center'}
...
```{r permanova-02, eval = TRUE, echo = FALSE}
# adonis2(D ~ grp, permu=99)$aov.tab
```
...
```{r permanova-03, eval = FALSE, echo = TRUE}
permutest(betadisper(D, grp), pairwise=TRUE, permu=99)
```

```{r permanova-04, eval = TRUE, echo = FALSE}
...
```{r permanova-05, eval = TRUE, echo = TRUE}
...
###   (traits already scaled 0-1)
Dt  <- dist(tra, method='euc') 
### calculate trait SES of mean pairwise 
###   distances in sites
ses <- picante::ses.mpd(spe, Dt, 
         null.model='richness')
### plot SES across a gradient
...
.pull-right.w50[
```{r cwm-06, eval = TRUE, echo = FALSE, fig.height = 5.75, fig.width = 5.75}
s <- scores(m$points, choices=1:2)
plot(s, asp=1, type='n', main='red = annual')
orditorp(s, disp='si', labels=rownames(s), 
         col=c(1,2)[tra$annual+1])
...
.pull-right.w50[
```{r rlq-02, eval = TRUE, echo = FALSE, fig.height = 7.75, fig.width=5.75}
...
```{r rlq-03, eval = TRUE, echo = TRUE, fig.height = 5.75, fig.width=5.75}
randtest(r)
```
...
```{r phyloplot-01, eval = TRUE, echo = TRUE, fig.height = 5.75, fig.width=5.75}
...
```{r mrm-01, eval = TRUE, echo = TRUE, fig.height = 5.75, fig.width=5.75}
...
```{r pcnm-01, eval = TRUE, echo = TRUE, fig.height = 5.75, fig.width=5.75}
